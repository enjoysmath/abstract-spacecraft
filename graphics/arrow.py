...
class Arrow(GraphicsShape, Connector, Bounded, DragDroppable, Containable, HasContextMenu, Deletable):
    ...
    def __init__(self, text:str=None, ctrl_points=None):
        ...
        self.setFlags(self.ItemIsSelectable | self.ItemIsFocusable | self.ItemSendsScenePositionChanges \
    ...
    def update_control_point_positions(self):
        if not self._updatingPointPos:
            ...
            if not self.is_bezier:
                a = source.closest_boundary_pos_to(dest)
                
                if isinstance(dest, Arrow):
                    b = dest.closest_boundary_pos_to(source)      
                else:
                ...
                if isinstance(dest, Arrow):
                    ...
                    b = closest_point_on_path(self._points[2].mapFromItem(source, a), self._points[2].paint_shape())
