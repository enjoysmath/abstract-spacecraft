...
𝟀	𝟁	𝟂	𝟃	𝟄	𝟅	𝟆	𝟇	𝟈	𝟉	𝟊	𝟋			
𝟎	𝟏   𝟐	𝟑	𝟒	𝟓	𝟔	𝟕	𝟖	𝟗	𝟘	𝟙	𝟚	𝟛	𝟜	𝟝	
𝟞	𝟟   𝟠	𝟡	𝟢	𝟣	𝟤	𝟥	𝟦	𝟧	𝟨	𝟩	𝟪	𝟫	𝟬	𝟭	
𝟮	𝟯   𝟰	𝟱	𝟲	𝟳	𝟴	𝟵	𝟶	𝟷	𝟸	𝟹	𝟺	𝟻	𝟼	𝟽	
𝟾	𝟿   
'''
...
half_width_and_full_width_forms = \
'''
	！	＂	＃	＄	％	＆	＇	（	）	＊	＋	，	－	．	／
０	１	２	３	４	５	６	７	８	９	：	；	＜	＝	＞	？
...
class UnicodeSupport:
    def __init__(self):
        ...
        for key, value in globals().items():
            if isinstance(value, str) and key not in ['__cached__', '__name__', '__file__', '__package__']:
                self.add_symbols(value)

    unicode_symbols_regex = re.compile(r'[ \t\n\r\u200e]')
            
    def add_symbols(self, symbols:str):
        symbols = self.unicode_symbols_regex.split(symbols)
        symbols = filter(lambda x: len(x) != 0, symbols)
        symbols = list(symbols)
        
        if symbols:
            symbols.sort()
            ordStart = ord(symbols[0])
            ordEnd = ordStart + 1
            ...
            for k in range(1, len(symbols)):
                ...
                if c == chr(ordEnd + 1):
                    ...
                    if k == len(symbols) - 1:
                        ...
                        ordStart = ord(symbols[k])
                        ordEnd = ordStart + 1
                else:
                    for d in range(ordStart, ordEnd):
                        d = chr(d)
                        if d in self._symbolsDict:
                            exist = self._symbolsDict[d]
                            self._symbolsDict[d] = (min(exist[0], ordStart), 
                                                    max(exist[1], ordEnd))
                        else:                            
                            self._symbolsDict[d] = (ordStart, ordEnd)
                    if k != len(symbols) - 1:
                        ordStart = ord(symbols[k+1])
                        ordEnd = ord(symbols[k+1]) + 1
                            
    def contiguous_range(self, c:chr):
        return self._symbolsDict[c]
